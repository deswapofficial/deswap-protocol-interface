...
export default function HeaderConnect() {
        ...
        if (netId) {
            if (netId === 4 || netId === 137) {
                if (netId === 137 && process.env.REACT_APP_ENV === 'prod') {
                    dispatch(showToastMessage(`You are currently visiting the Etherium Mainnet Network. Please change your metamask to access the Etherium Mainnet Network`, 'error'))
                } else if (netId === 4 && process.env.REACT_APP_ENV === 'dev') {
                    dispatch(showToastMessage(`You are currently visiting the Rinkeyby Tesetnet Network. Please change your metamask to access the Rinkeby Testnet Network`, 'error'))
                } else {
                    dispatch(
                        setSetting({
                        wrongNetwork: false
                ...
                dispatch(showToastMessage(`Dswap is only supported on Etherium Network. Please confirm you installed Metamask and selected Etherium Network`, 'error'))
            }
            dispatch(
                setSetting({
                    wrongNetwork: true
    ...
    //   };
    return (
        <div className={ openConnect ? "header__item header__item_notifications active" : "header__item header__item_notifications" } style={{marginLeft: "36px"}}>
            <button className="header__head  pop-icon before-none" onClick={handleOpen}>
            ...
            <div className="header__body header-body-2">
                ...
                <div className="header__notification">
                    <div className="header__details">
                        <button className="card__btn btn btn_blue">
                            Connect Wallet
                        </button>
                    </div>
                </div>
