...
const vaultContract = new web3.eth.Contract(YaiVaultAbi, constants.REACT_APP_YaiVaultProxyAddress);
const YAIContract = new web3.eth.Contract(YAIabi, constants.REACT_APP_YAIAddress);
const DawContract = new web3.eth.Contract(YAIabi, constants.REACT_APP_DAWAddress);
...
export const YAIApprove = (myAddress) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        if(web3 && web3.currentProvider){
            YAIContract.methods
                .approve(constants.REACT_APP_YaiVaultProxyAddress, amount)
                .send({from : myAddress})
                .then((data) => {
                    resolve(data)
...
export const YAIAllowance = (myAddress) => {
    return new Promise((resolve, reject) => {
        if(web3 && web3.currentProvider){
            YAIContract.methods
                .allowance(myAddress, constants.REACT_APP_YaiVaultProxyAddress)
                .call({from : myAddress})
                .then((data) => {
                    resolve(data)
                }).catch(error => reject(error));
...
export const YAIBalanceOfAddress = myAddress => {
    if(!myAddress) myAddress = constants.REACT_APP_YaiVaultProxyAddress
    return new Promise((resolve, reject) => {
        if(web3 && web3.currentProvider){
            YAIContract.methods
                .balanceOf(myAddress)
...
export const PendingRewards = () => {
    return new Promise((resolve, reject) => {
        if(web3 && web3.currentProvider){
            DawContract.methods
                .balanceOf(constants.REACT_APP_YaiVaultProxyAddress)
                .call()
                .then((data) => {
                    resolve(data)
                }).catch(error => reject(error));
